...
public class TraceAop {
    ...
    public static void methodAfter(long startTime) {
        ...
        if (spanStack == null || spanStack.empty()) {
            ...
            //完全出栈发送数据
            if (!ThreadLocalData.requesturl.get().isEmpty() && spanStack.empty()) {
                System.out.println("size:" + trace.size() + "---" + methodDataMap.size());

                executor.submit(new SendTraceTask(ClientConstant.CONS_INFO.get(2), trace, methodDataMap, ThreadLocalData.requesturl.get(),ThreadLocalData.sqlData.get(),com.weaver.agent.asm.filecheck.ThreadLocalData.getFiledelSpan().get()));
            }
        }
    }
}
